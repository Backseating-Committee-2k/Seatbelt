...
[[nodiscard]] static StringView c_string_view(const std::string_view view) {
    return string_view_from_pointers(view.data(), view.data() + view.length());
}

[[nodiscard]] static std::string c_string_view_to_string(const StringView view) {
    return std::string{ view.data, view.length };
}

[[nodiscard]] static std::string_view c_string_view_to_string_view(const StringView view) {
...
parse_bssembly(const std::string_view filename, std::string_view source, Location location) {
    ...
    const auto source_file = SourceFile{
        .filename{ c_string_view(filename) },
        .source{ c_string_view(source_string) },
    };
    auto tokens = tokenize(source_file, &constants);
    ...
    while (true) {
        ...
        // as well
        auto mnemonic_string =
                Utils::to_upper(std::string_view{ tokens.data[i].string_view.data, tokens.data[i].string_view.length });
        ...
        while (needs_more_arguments
            ...
            switch (tokens.data[i].type) {
                ...
                case TOKEN_TYPE_WORD_LITERAL: {
                    bool success;
                    Word result;
                    word_from_token(&tokens.data[i], &success, &result);
                ...
                default:
                    Error::error(
                            ...
                            fmt::format(
                                    "unexpected token type \"{}\" (\"{}\")", magic_enum::enum_name(tokens.data[i].type),
                                    c_string_view_to_string_view(tokens.data[i].string_view)
                            )
                    );
                    break;
